...
public final class RequestUtil {
    ...
    public static String getScheme(HttpServletRequest request) {
        ...
        if (StrUtil.isNotBlank(referer)) {
            if (referer.startsWith(HTTP_SCHEME_FULL)) {
                scheme = HTTP_SCHEME_START;
            } else if (referer.startsWith(HTTPS_SCHEME_FULL)) {
                scheme = HTTPS_SCHEME_START;
            }
        }

        if (StrUtil.isBlank(scheme)) {
            scheme = getHeader(request, HTTP_HEADER_HTTP_X_FORWARDED_PROTO);
        }

        if (!HTTP_SCHEME_START.equalsIgnoreCase(scheme) && !HTTPS_SCHEME_START.equalsIgnoreCase(scheme)) {
